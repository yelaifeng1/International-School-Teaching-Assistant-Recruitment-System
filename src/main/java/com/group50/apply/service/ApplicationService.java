...
public class ApplicationService {
    ...
    private static final Type APP_LIST_TYPE = new TypeToken<List<Application>>() {}.getType();
    ...
    public List<Application> getAll() {
        try {
            ...
            try (Reader reader = new InputStreamReader(new FileInputStream(f), StandardCharsets.UTF_8)) {
                List<Application> list = GSON.fromJson(reader, APP_LIST_TYPE);
    ...
    /** 将列表写回 JSON 文件 */
    private void writeAll(List<Application> all) {
        try (Writer writer = new OutputStreamWriter(new FileOutputStream(getDataFile()), StandardCharsets.UTF_8)) {
            GSON.toJson(all, APP_LIST_TYPE, writer);
        } catch (Exception e) {
